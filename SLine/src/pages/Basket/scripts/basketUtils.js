export async function updateQuantityWithCheck(user, userId, storeId, productId, quantity, setNotification, showNotification, fetchBasketItems) {
    try {
        const response = await fetch(`http://45.142.122.22/Catalog/Warehouse/${storeId}/${productId}`, {
            headers: {
        ...
        if (response.ok) {
            ...
            const updateResponse = await fetch(`http://45.142.122.22/Basket/UpdateQuantity`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`,
...
export async function removeProduct(user, userId, groupedItems, storeId, productId, setNotification, showNotification, fetchBasketItems) {
    try {
        ...
        const response = await fetch(`http://45.142.122.22/Basket/RemoveProduct`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${user.token}`,
...
export async function removeBasket(userId, user, setNotification, showNotification, fetchBasketItems) {
    ...
    try {
        const response = await fetch(`http://45.142.122.22/Basket/RemoveBasket/${userId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${user.token}`,
