...
function ProductDetails() {
    ...
    const fetchProductData = async () => {
        try {
            const response = await fetch(`https://sline.site/api/Catalog/Product/${productId}`);
            const productData = await response.json();

            setProduct(productData);
            setProductName(productData.productName);

            const categoryResponse = await fetch(`https://sline.site/api/Catalog/Categories`);
            ...
            const priceResponse = await fetch(`https://sline.site/api/Catalog/Warehouse/${productId}`);
    ...
    const handleAddToCart = async (event) => {
        ...
        if (userId) {
            try {
                const response = await fetch(`https://sline.site/api/Profile/GetAllInfo?userId=${userId}`);
    ...
    const fetchStores = async () => {
        try {
            const response = await fetch(`https://sline.site/api/Basket/AvailableStores/${product.productId}`);
            if (response.ok) {
                const data = await response.json();
                setStores(data);
            } else {
