...
function ProductDetails() {
    ...
    const fetchProductData = async () => {
        try {
            const response = await fetch(`http://localhost:7036/Catalog/Product/${productId}`);
            const productData = await response.json();

            setProduct(productData);
            setProductName(productData.productName);

            const categoryResponse = await fetch(`http://localhost:7036/Catalog/Categories`);
            ...
            const priceResponse = await fetch(`http://localhost:7036/Catalog/Warehouse/${productId}`);
    ...
    const handleAddToCart = async (event) => {
        ...
        if (userId) {
            try {
                const response = await fetch(`http://localhost:7036/Profile/GetAllInfo?userId=${userId}`);
