...
function OrderItem() {
    ...
    async function fetchOrders() {
        try {
            const response = await fetch(`http://localhost:7036/Order/GetOrders/${userId}`, {
                method: 'GET',
                headers: {
    ...
    async function updateOrderStatus(orderId, statusId) {
        ...
        try {
            const response = await fetch(`http://localhost:7036/Order/UpdateOrderStatus`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user ? user.token : ''}`,
    ...
    async function cancelOrder(orderId) {
        ...
        try {
            const response = await fetch(`http://localhost:7036/Order/CancelOrder?orderId=${orderId}&userId=${userId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user ? user.token : ''}`,
