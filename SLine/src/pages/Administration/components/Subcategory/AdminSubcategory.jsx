...
function AdminSubCategory() {
    ...
    const refreshSubCategories = () => {
        fetch('http://45.142.122.22/GetAllSubCategories')
    ...
    const refreshCategories = () => {
        fetch('http://45.142.122.22/GetAllCategories')
            .then(response => response.json())
    ...
    const handleSubmit = () => {
        const url = currentSubCategory.subCategoryId ? 'http://45.142.122.22/UpdateSubCategory' : 'http://45.142.122.22/AddSubCategory';
    ...
    const handleDelete = (subCategoryId) => {
        if (window.confirm('Вы уверены, что хотите удалить эту подкатегорию?')) {
            fetch(`http://45.142.122.22/DeleteSubCategory/${subCategoryId}`, {
                method: 'DELETE',
            })
