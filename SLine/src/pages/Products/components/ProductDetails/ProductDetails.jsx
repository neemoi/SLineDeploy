...
function ProductDetails() {
    ...
    const fetchProductData = async () => {
        try {
            const response = await fetch(`http://localhost:7036/api/Catalog/Product/${productId}`);
            ...
            const priceResponse = await fetch(`http://localhost:7036/api/Catalog/Warehouse/${productId}`);
    ...
    const handleAddToCart = async (event) => {
        ...
        if (userId) {
            try {
                const response = await fetch(`http://localhost:7036/api/Profile/GetAllInfo?userId=${userId}`);
