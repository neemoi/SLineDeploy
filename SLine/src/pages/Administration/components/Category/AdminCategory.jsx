...
function AdminCategory() {
    ...
    const handleSubmit = () => {
        const url = currentCategory.categoryId ? `http://localhost:7036/UpdateCategory` : `http://localhost:7036/AddCategory`;
    ...
    const handleDelete = (categoryId) => {
        if (window.confirm('Вы уверены, что хотите удалить эту категорию?')) {
            fetch(`http://localhost:7036/DeleteCategory/${categoryId}`, {
                method: 'DELETE',
            })
