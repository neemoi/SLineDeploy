...
function ProductDetails() {
    ...
    const fetchProductData = async () => {
        try {
            const response = await fetch(`http://http://109.107.189.127/api//api/Catalog/Product/${productId}`);
            ...
            const priceResponse = await fetch(`http://http://109.107.189.127/api//api/Catalog/Warehouse/${productId}`);
    ...
    const handleAddToCart = async (event) => {
        ...
        if (userId) {
            try {
                const response = await fetch(`http://http://109.107.189.127/api//api/Profile/GetAllInfo?userId=${userId}`);
    ...
    const fetchStores = async () => {
        try {
            const response = await fetch(`http://http://109.107.189.127/api//api/Basket/AvailableStores/${product.productId}`);
            if (response.ok) {
                const data = await response.json();
                setStores(data);
            } else {
