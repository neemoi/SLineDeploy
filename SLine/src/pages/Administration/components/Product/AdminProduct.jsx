...
function AdminProduct() {
    ...
    const fetchCategories = () => {
        fetch('http://http://109.107.189.127/api//api/Catalog/Categories')
            .then(response => response.json())
            .then(data => {
                setCategories(data);
            })
    ...
    const handleAddOrUpdateProduct = () => {
        ...
        const url = currentProduct.productId ? 'http://http://109.107.189.127/api//api/UpdateProduct' : 'http://http://109.107.189.127/api//api/AddProduct';
    ...
    const deleteProduct = (productId) => {
        if (window.confirm('Вы уверены, что хотите удалить этот продукт?')) {
            fetch(`http://http://109.107.189.127/api//api/DeleteProduct/${productId}`, {
                method: 'DELETE',
            })
                .then(() => fetchProducts())
