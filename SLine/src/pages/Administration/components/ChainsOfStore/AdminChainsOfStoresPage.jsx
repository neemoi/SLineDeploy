...
function ChainsOfStoresPage() {
    ...
    const handleAddOrUpdateChain = () => {
        ...
        const url = currentChain.chainId === 0
            ? ' http://www.sline.site/api/Admin/Store/AddChainOfStore'
            : ' http://www.sline.site/api/Admin/Store/UpdateChainOfStore';
    ...
    const handleDeleteChain = (chainId) => {
        if (window.confirm('Вы уверены, что хотите удалить эту сеть магазинов?')) {
            fetch(` http://www.sline.site/api/Admin/Store/DeleteChainOfStore/${chainId}`, {
                method: 'DELETE'
            })
            .then(response => response.json())
            .then(data => {
