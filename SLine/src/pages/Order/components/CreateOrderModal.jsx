...
function OrderModal({ isOpen, onClose, storeId, userId, groupedItems }) {
    ...
    useEffect(() => {
        const fetchDeliveryOptions = async () => {
            try {
                const response = await fetch(`http://localhost:7036/api/api/Order/Delivery/${storeId}`);
                if (response.ok) {
        ...
        const fetchPaymentTypes = async () => {
            try {
                const response = await fetch('http://localhost:7036/api/Order/PaymentType');
                if (response.ok) {
                    const data = await response.json();
    ...
    const createOrder = async () => {
        ...
        try {
            const response = await fetch('http://localhost:7036/api/Order/Create', {
                method: 'POST',
                headers: {
